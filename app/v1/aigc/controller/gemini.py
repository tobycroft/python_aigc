...
async def text():
    ...
    data = tuuz.Database.Db(self.db).table("ai_project").where('token', token).find()
    gemini = tuuz.Database.Db(self.db).table("ai_gemini").where('project_name', data["name"]).find()
    if gemini is None:
    ...
    tuuz.Database.Db(self.db).table("ai_gemini").where('project_name', data["name"]).update({"rcid": chat.rcid, "cid": chat.cid, "rid": chat.rid})
    tuuz.Database.Db(self.db).table("log").insert({"type": "gemini", "project": data["name"], "ask": text, "reply": response.text})
    print(response)
...
async def test():
    data = tuuz.Database.Db(self.db).table("log").where("type", "gemini").find()
    if data is None:
        tuuz.Ret.fail(404)
