...
class FastgptModel(BaseModel):
    ...
    def api_update_byUidAndId(self, uid, id, name, key, model, detail):
        return Database.Db().table(self.Table).where("uid", uid).where("id", id).update({"name": name, "key": key, "model": model, "detail": detail})
