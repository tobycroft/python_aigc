...
#   `qianwen_id` int unsigned DEFAULT '0' COMMENT 'fastgpt表的id',
...
class QianwenRecordModel(BaseModel):
    Table = "ai_qianwen_record"
    ...
    def api_insert(self, qianwen_id, subtoken, chatId, send, reply, completion_tokens, prompt_tokens, total_tokens,
                   finish_reason, amount):
        return Database.Db(self.db).table(self.Table).insert({
            "qianwen_id": qianwen_id,
            "subtoken": subtoken,
            "chatId": chatId,
            "send": send,
            "reply": reply,
    ...
    def api_select_byFastgptIdAndChatId(self, qianwen_id, chatId):
        return Database.Db(self.db).table(self.Table).where("qianwen_id", qianwen_id).where("chatId", chatId).order("id asc").select()

    def api_find_byFastgptIdAndChatId(self, qianwen_id, chatId):
        return Database.Db(self.db).table(self.Table).where("qianwen_id", qianwen_id).where("chatId", chatId).order("id desc").find()

    def api_find_bySubtokenAndChatId(self, subtoken, chatId):
